...
public class FileAnalysis {
	...
	private void LoopFileTraversal() throws IOException{
		...
		fileQueue.addLast(filesArray);
		while(!(fileQueue.isEmpty())){
			try {
				for(File file:fileQueue.removeFirst() ){
					if (file.isDirectory()) {
						checkDuplicate(file);
						File[] filesArray1 = file.listFiles();
						if(!(filesArray1 == null)){
	 ...
	 */
	private void checkDuplicate(File file) throws IOException{
		try {
			Path path = Paths.get(file.getAbsolutePath());
			if(Files.isSymbolicLink(path) || !(Files.isReadable(path))){
				return;
			}
	...
	public static void main (String[] args) throws IOException {
		...
		try {
			...
			FileAnalysis fa = new FileAnalysis("/Volumes/TimeMachine");
